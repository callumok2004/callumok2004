...
function portArgs(service: Service) {
  if (!service.ports || !service.ports.length) return ''
  return service.ports.map(p => `-p ${p}`).join(' ')
}
...
function StartDockerContainer(service: Service) {
  ...
  const ports = portArgs(service)

  let command = ''
  if (fileExists(dockerfilePath)) {
    command = `docker build -t ${nameLower}-image ${workdir} && docker run -d --name ${name} ${ports} ${nameLower}-image ${service.command}`
  } else {
    command = `docker run -d --rm --name ${name} ${ports} -v ${workdir}:/app -w /app ${service.command}`
  }
